...
/**
 * Create a domain mapping from a domain to a VM port.
 * Uses *.style.dev subdomains which don't require DNS verification.
 */
export async function createDomainMapping(opts: {
  domain: string;
  vmId: string;
  vmPort: number;
}): Promise<{ domain: string; vmId: string; vmPort: number }> {
  const res = await fetch(`${FREESTYLE_API}/v1/domains/mappings`, {
    method: 'POST',
    headers: headers(),
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(`Domain mapping failed: ${res.status} ${JSON.stringify(err)}`);
  }
...
/**
 * List domain mappings for a domain.
 */
export async function listDomainMappings(domain: string) {
  const res = await fetch(`${FREESTYLE_API}/v1/domains/mappings?domain=${domain}`, {
    method: 'GET',
    headers: headers(),
  });
  if (!res.ok) throw new Error(`List mappings failed: ${res.status}`);
  return res.json();
}
 ...
 */
export async function deleteDomainMapping(domain: string) {
  const res = await fetch(`${FREESTYLE_API}/v1/domains/mappings`, {
    method: 'DELETE',
    headers: headers(),
    body: JSON.stringify({ domain }),
  });
  if (!res.ok) throw new Error(`Delete mapping failed: ${res.status}`);
  return res.json();
}
